...
import maze.mazerun as mazerun
...
def got_obstacles(name):
    """is responsible for printing out the coordinates of the obstacles"""
    global env
    if len(sys.argv) > 2 and sys.argv[2] != "--up=test_*.py":
        print(f"{name}: Loaded {sys.argv[2]}.")
    else:
...
def process_command(command, name):
    ...
    global run
    valid_ls = ["off","help","forward","back","right","left","sprint","replay","mazerun","reset"]
    move = command.split()
...
MAZERUN - allows the robot to attempt to complete a given maze
RESET - resets the postion of the robot but doesnt work well with mazerun
""")
        else:
...
def robot_movement(command, name):
    ...
    elif command.casefold() == "reset":
        return reset_postion(name)
    ...
    elif move[0].casefold() == "mazerun":
        # reset_postion(name)
        mazerun.mazerunner(name, command, sys.argv)
...
def turn_right(name):
    ...
    if direction < 3:
        ...
        direction = 0
    # print(f"this is used direction {direction}")
    move_y = 0 
    move_x = 0
...
def reset_postion(name):
    """resets the postion of the robot / send the robot to the (0,0) postion.
    doesnt work well with mazerun"""
    global direction
    direction = 0
    if env == False:
        world.text.world.x = 0
        world.text.world.y = 0
        world.text.world.track_position(0,0,name,False)
    else:
        world.turtle.world.reset_postion()
        world.turtle.world.track_position(0,0,name,False)
    # print(f"this is supposed direction {direction}")
    print(f"{name}: has moved to start point")
