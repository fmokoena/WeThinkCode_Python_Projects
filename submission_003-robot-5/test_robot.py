...
""")
    ...
    @patch("sys.stdin",StringIO("BMO\nmazerun\noff\n"))
    def test_mazerun_top(self):
        """tests that the mazerun function works correctly"""
        obstacles.random.randint = lambda a, b: 1
        with patch('sys.stdout', new = StringIO()) as fake_out:
            robot.robot_start()
        self.assertEqual(fake_out.getvalue() , """What do you want to name your robot? BMO: Hello kiddo!
...
BMO: What must I do next? BMO: Shutting down..\n""") 
    ...
    def test_mazerun_left(self):
        """tests that the mazerun function works correcty with the mazerun left command"""
        obstacles.random.randint = lambda a, b: 1
        with patch('sys.stdout', new = StringIO()) as fake_out:
            robot.robot_start()
        self.assertEqual(fake_out.getvalue() , """What do you want to name your robot? BMO: Hello kiddo!
...
BMO: What must I do next? > BMO starting maze run..
 > BMO turned left.
 > BMO now at position (0,0).
 > BMO moved forward by 100 steps.
 > BMO now at position (-100,0).
BMO: I am at the left edge.
BMO: What must I do next? BMO: Shutting down..\n""") 


    @patch("sys.stdin",StringIO("BMO\nmazerun right\noff\n"))
    def test_mazerun_right(self):
        """tests that the mazerun function works correcty with the mazerun right command"""
        obstacles.random.randint = lambda a, b: 1
        with patch('sys.stdout', new = StringIO()) as fake_out:
            robot.robot_start()
        self.assertEqual(fake_out.getvalue() , """What do you want to name your robot? BMO: Hello kiddo!
